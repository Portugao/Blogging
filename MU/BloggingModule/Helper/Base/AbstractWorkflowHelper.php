...
abstract class AbstractWorkflowHelper
{
    ...
    public function collectAmountOfModerationItems()
    {
        ...
        $state = 'waiting';
        $objectType = 'post';
        if ($this->permissionApi->hasPermission('MUBloggingModule:' . ucfirst($objectType) . ':', '::', ACCESS_ADD)) {
            $amount = $this->getAmountOfModerationItems($objectType, $state);
            if ($amount > 0) {
                $amounts[] = [
                    'aggregateType' => 'postsApproval',
                    'description' => $this->translator->__('Posts pending approval'),
                    'amount' => $amount,
                    'objectType' => $objectType,
                    'state' => $state,
                    'message' => $this->translator->_fn('One post is waiting for approval.', '%amount% posts are waiting for approval.', $amount, ['%amount%' => $amount])
                ];
        
                $this->logger->info('{app}: There are {amount} {entities} waiting for approval.', ['app' => 'MUBloggingModule', 'amount' => $amount, 'entities' => 'posts']);
            }
        }
