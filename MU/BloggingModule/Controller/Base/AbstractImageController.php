...
abstract class AbstractImageController extends AbstractController
{
    ...
    protected function indexInternal(Request $request, $isAdmin = false)
    {
        ...
        return $this->redirectToRoute('mubloggingmodule_image_' . $templateParameters['routeArea'] . 'view');
    }
    /**
    ...
    protected function viewInternal(Request $request, $sort, $sortdir, $pos, $num, $isAdmin = false)
    {
        ...
        $sortableColumns = new SortableColumns($this->get('router'), 'mubloggingmodule_image_' . ($isAdmin ? 'admin' : '') . 'view', 'sort', 'sortdir');
     ...
     * @ParamConverter("image", class="MUBloggingModule:ImageEntity", options = {"id" = "id", "repository_method" = "selectById"})
    ...
    protected function editInternal(Request $request, $isAdmin = false)
    {
        ...
        $formHandler = $this->get('mu_blogging_module.form.handler.image');
    ...
    protected function deleteInternal(Request $request, ImageEntity $image, $isAdmin = false)
    {
        ...
        $logArgs = ['app' => 'MUBloggingModule', 'user' => $this->get('zikula_users_module.current_user')->get('uname'), 'entity' => 'image', 'id' => $image->createCompositeIdentifier()];
        ...
        $redirectRoute = 'mubloggingmodule_image_' . ($isAdmin ? 'admin' : '') . 'view';
        ...
        $form = $this->createForm('MU\BloggingModule\Form\DeleteEntityType', $image);
    ...
    protected function handleSelectedEntriesActionInternal(Request $request, $isAdmin = false)
    {
        ...
        foreach ($items as $itemId) {
            ...
            try {
                ...
                $logger->error('{app}: User {user} tried to execute the {action} workflow action for the {entity} with id {id}, but failed. Error details: {errorMessage}.', ['app' => 'MUBloggingModule', 'user' => $userName, 'action' => $action, 'entity' => 'image', 'id' => $itemId, 'errorMessage' => $e->getMessage()]);
            }
            ...
            if ($action == 'delete') {
                $this->addFlash('status', $this->__('Done! Item deleted.'));
                $logger->notice('{app}: User {user} deleted the {entity} with id {id}.', ['app' => 'MUBloggingModule', 'user' => $userName, 'entity' => 'image', 'id' => $itemId]);
            } else {
                $this->addFlash('status', $this->__('Done! Item updated.'));
                $logger->notice('{app}: User {user} executed the {action} workflow action for the {entity} with id {id}.', ['app' => 'MUBloggingModule', 'user' => $userName, 'action' => $action, 'entity' => 'image', 'id' => $itemId]);
            }
            ...
            if ($action != 'delete') {
                ...
                $url = new RouteUrl('mubloggingmodule_image_' . /*($isAdmin ? 'admin' : '') . */'display', $urlArgs);
            }
            $hookHelper->callProcessHooks($entity, $hookType, $url);
        }
        
        return $this->redirectToRoute('mubloggingmodule_image_' . ($isAdmin ? 'admin' : '') . 'index');
    }
}
