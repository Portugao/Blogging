...
abstract class AbstractEditHandler extends EditHandler
{
    ...
    public function processForm(array $templateParameters = [])
    {
        ...
        if ('create' == $this->templateParameters['mode']) {
            if (!$this->modelHelper->canBeCreated($this->objectType)) {
                $this->requestStack->getCurrentRequest()->getSession()->getFlashBag()->add('error', $this->__('Sorry, but you can not create the post yet as other items are required which must be created before!'));
    ...
    {
        return $this->formFactory->create(PostType::class, $this->entityRef, $this->getFormOptions());
    }
    ...
    protected function getFormOptions()
    {
        $options = [
            ...
            'allow_moderation_specific_creator' => $this->variableApi->get('MUBloggingModule', 'allowModerationSpecificCreatorFor' . $this->objectTypeCapital, false),
            'allow_moderation_specific_creation_date' => $this->variableApi->get('MUBloggingModule', 'allowModerationSpecificCreationDateFor' . $this->objectTypeCapital, false),
        ];
    ...
    protected function getDefaultReturnUrl(array $args = [])
    {
        ...
        if ($objectIsPersisted) {
            // redirect to the detail page of treated post
    ...
    protected function getRedirectUrl(array $args = [])
    {
        ...
        if ($session->has('mubloggingmodule' . $this->objectTypeCapital . 'Referer')) {
            $this->returnTo = $session->get('mubloggingmodule' . $this->objectTypeCapital . 'Referer');
