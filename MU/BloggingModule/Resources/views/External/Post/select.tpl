...
<div class="row">
    <div class="col-sm-8">
        ...
        {if $properties ne null && is_array($properties)}
            ...
            {foreach key='propertyName' item='propertyId' from=$properties}
                <div class="form-group">
                    {assign var='hasMultiSelection' value=$categoryHelper->hasMultipleSelection('post', $propertyName)}
                    ...
                    <div class="col-sm-9">
                        {selector_category name="`$baseID`_`$categorySelectorName``$propertyName`" field='id' selectedValue=$catIds.$propertyName|default:null categoryRegistryModule='MUBloggingModule' categoryRegistryTable="`$objectType`Entity" categoryRegistryProperty=$propertyName defaultText=$lblDefault editLink=false multipleSize=$categorySelectorSize cssClass='form-control'}
                    </div>
                </div>
            {/foreach}
            {/nocache}
        {/if}
        <div class="form-group">
            <label for="{$baseID}Id" class="col-sm-3 control-label">{gt text='Post'}:</label>
            <div class="col-sm-9">
            ...
            <div class="col-sm-9">
                <div class="input-group">
                    ...
                    <span class="input-group-btn">
                        <input type="button" id="mUBloggingModuleSearchGo" name="gosearch" value="{gt text='Filter'}" class="btn btn-default" />
                    </span>
                </div>
            </div>
        </div>
...
/* <![CDATA[ */
    ( function($) {
        $(document).ready(function() {
            mUBloggingModule.itemSelector.onLoad('{{$baseID}}', {{$selectedId|default:0}});
        });
    })(jQuery);
/* ]]> */
</script>
