...
abstract class AbstractThirdPartyListener implements EventSubscriberInterface
{
    ...
    {
        $collection = new Container('MUBloggingModule');
        ...
        if (count($amounts) > 0) {
            foreach ($amounts as $amountInfo) {
                ...
                $route = 'mubloggingmodule_' . strtolower($amountInfo['objectType']) . '_adminview';
                $routeArgs = [
     ...
     * MUBloggingModule will use this to add a javascript helper to add custom items.
     *
     * You can access general data available in the event.
     *
     * The event name:
    ...
    public function getEditorHelpers(EditorHelperEvent $event)
    {
        ...
        $targetDir = 'web/modules/mublogging';
        $finder = new Finder();
        if (!$this->filesystem->exists($targetDir)) {
            $this->filesystem->mkdir($targetDir, 0777);
            if (is_dir($originDir = 'modules/MU/BloggingModule/Resources/public')) {
                $this->filesystem->mirror($originDir, $targetDir, Finder::create()->in($originDir));
            }
            if (is_dir($originDir = 'modules/MU/BloggingModule/Resources/scribite')) {
                $targetDir .= '/scribite';
                $this->filesystem->mkdir($targetDir, 0777);
                $this->filesystem->mirror($originDir, $targetDir, Finder::create()->in($originDir));
            }
        ...
        $event->getHelperCollection()->add(
            [
                ...
                'type' => 'javascript',
                'path' => $this->requestStack->getCurrentRequest()->getBasePath() . '/web/modules/mublogging/js/MUBloggingModule.Finder.js'
            ]
        );
    }
    ...
    public function getCKEditorPlugins(GenericEvent $event)
    {
        $event->getSubject()->add([
            'name' => 'mubloggingmodule',
            'path' => $this->requestStack->getCurrentRequest()->getBasePath() . '/web/modules/mublogging/scribite/CKEditor/mubloggingmodule/',
            'file' => 'plugin.js',
            'img' => 'ed_mubloggingmodule.gif'
        ]);
    }
    ...
    public function getQuillPlugins(GenericEvent $event)
    {
        $event->getSubject()->add([
            'name' => 'mubloggingmodule',
            'path' => $this->requestStack->getCurrentRequest()->getBasePath() . '/web/modules/mublogging/scribite/Quill/mubloggingmodule/plugin.js'
        ]);
    }
    ...
    public function getSummernotePlugins(GenericEvent $event)
    {
        $event->getSubject()->add([
            'name' => 'mubloggingmodule',
            'path' => $this->requestStack->getCurrentRequest()->getBasePath() . '/web/modules/mublogging/scribite/Summernote/mubloggingmodule/plugin.js'
        ]);
    }
    ...
    public function getTinyMcePlugins(GenericEvent $event)
    {
        $event->getSubject()->add([
            'name' => 'mubloggingmodule',
            'path' => $this->requestStack->getCurrentRequest()->getBasePath() . '/web/modules/mublogging/scribite/TinyMce/mubloggingmodule/plugin.js'
        ]);
    }
}
