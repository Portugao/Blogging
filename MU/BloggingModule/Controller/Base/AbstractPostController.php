...
abstract class AbstractPostController extends AbstractController
{
    ...
    protected function viewInternal(Request $request, $sort, $sortdir, $pos, $num, $isAdmin = false)
    {
        ...
        $filteredEntities = [];
        foreach ($templateParameters['items'] as $post) {
    ...
    {
        $post = $this->get('mu_blogging_module.entity_factory')->getRepository('post')->selectBySlug($slug);
        if (null === $post) {
            throw new NotFoundHttpException($this->__('No such post found.'));
        }
    ...
    protected function deleteInternal(Request $request, $slug, $isAdmin = false)
    {
        ...
        $form = $this->createForm(DeletionType::class, $post);
        if ($post->supportsHookSubscribers()) {
            $hookHelper = $this->get('mu_blogging_module.hook_helper');
        
            // Call form aware display hooks
            $formHook = $hookHelper->callFormDisplayHooks($form, $post, FormAwareCategory::TYPE_DELETE);
        }
        ...
        if ($form->handleRequest($request)->isValid()) {
            if ($form->get('delete')->isClicked()) {
                if ($post->supportsHookSubscribers()) {
                    ...
                    $validationErrors = $hookHelper->callValidationHooks($post, UiHooksCategory::TYPE_VALIDATE_DELETE);
                    if (count($validationErrors) > 0) {
                        ...
                        if ($post->supportsHookSubscribers()) {
                            ...
                            $hookHelper->callFormProcessHooks($form, $post, FormAwareCategory::TYPE_PROCESS_DELETE);
                        
                            // Let any ui hooks know that we have deleted the post
                            $hookHelper->callProcessHooks($post, UiHooksCategory::TYPE_PROCESS_DELETE);
                        }
    ...
    protected function handleSelectedEntriesActionInternal(Request $request, $isAdmin = false)
    {
        ...
        foreach ($items as $itemId) {
            ...
            try {
                ...
                $logger->error('{app}: User {user} tried to execute the {action} workflow action for the {entity} with id {id}, but failed. Error details: {errorMessage}.', ['app' => 'MUBloggingModule', 'user' => $userName, 'action' => $action, 'entity' => 'post', 'id' => $itemId, 'errorMessage' => $exception->getMessage()]);
            }
