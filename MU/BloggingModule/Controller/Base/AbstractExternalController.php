...
abstract class AbstractExternalController extends AbstractController
{
    ...
    public function displayAction(Request $request, $objectType, $id, $source, $displayMode)
    {
        ...
        if (!$this->get('mu_blogging_module.permission_helper')->mayRead($entity)) {
            return '';
        }
        ...
        $contextArgs = ['controller' => 'external', 'action' => 'display'];
        $templateParameters = $this->get('mu_blogging_module.controller_helper')->addTemplateParameters($objectType, $templateParameters, 'controllerAction', $contextArgs);
        
        return $this->render('@MUBloggingModule/External/' . ucfirst($objectType) . '/' . $template, $templateParameters);
    }
    ...
    public function finderAction(Request $request, $objectType, $editor, $sort, $sortdir, $pos = 1, $num = 0)
    {
        ...
        $cssAssetBag->add($assetHelper->resolve('@MUBloggingModule:css/style.css'));
        $cssAssetBag->add([$assetHelper->resolve('@MUBloggingModule:css/custom.css') => 120]);
        
        $listEntriesHelper = $this->get('mu_blogging_module.listentries_helper');
        ...
        if (!in_array($objectType, $activatedObjectTypes)) {
            ...
            $redirectUrl = $this->get('router')->generate('mubloggingmodule_external_finder', ['objectType' => array_shift($activatedObjectTypes), 'editor' => $editor]);
        
            return new RedirectResponse($redirectUrl);
        }
        
        if (!$this->get('mu_blogging_module.permission_helper')->hasComponentPermission($objectType, ACCESS_COMMENT)) {
            throw new AccessDeniedException();
        }
        ...
        if ('' != $searchTerm) {
            $qb = $this->get('mu_blogging_module.collection_filter_helper')->addSearchFilter($objectType, $qb, $searchTerm);
        }
        ...
        $output = $this->renderView('@MUBloggingModule/External/' . ucfirst($objectType) . '/find.html.twig', $templateParameters);
