 ...
 */
function mUBloggingInitUserField(fieldName, getterName)
{
    ...
    jQuery('#' + fieldName + 'Selector').typeahead({
        ...
        source: function (query, syncResults, asyncResults) {
            ...
            jQuery.getJSON(Routing.generate('mubloggingmodule_ajax_' + getterName.toLowerCase(), { fragment: query }), function( data ) {
 ...
 */
function mUBloggingResetDateField(fieldName)
{
...
function mUBloggingInitDateField(fieldName)
{
    jQuery('#' + fieldName + 'ResetVal').click( function (event) {
        event.preventDefault();
        mUBloggingResetDateField(fieldName);
    }).removeClass('hidden');
}
