...
namespace MU\BloggingModule\Form\Handler\Common\Base;
...
abstract class AbstractEditHandler
{
        ...
        if ($this->templateParameters['mode'] == 'edit') {
            ...
            if (null !== $entity) {
                if (true === $this->hasPageLockSupport && $this->kernel->isBundle('ZikulaPageLockModule') && null !== $this->lockingApi) {
                    ...
                    $lockName = 'MUBloggingModule' . $this->objectTypeCapital . $entity->getKey();
                    $this->lockingApi->addLock($lockName, $this->getRedirectUrl(['commandName' => '']));
                }
                if (!$this->permissionHelper->mayEdit($entity)) {
                    throw new AccessDeniedException();
                }
                if (in_array($this->objectType, ['post'])) {
                    // map display return urls to redirect codes because slugs may change
                    $routePrefix = 'mubloggingmodule_' . $this->objectTypeLower . '_';
    ...
    public function handleCommand(array $args = [])
    {
        ...
        if ($entity->supportsHookSubscribers()) {
            ...
            if ($hasDisplayAction && $action != 'delete') {
                ...
                $routeUrl = new RouteUrl('mubloggingmodule_' . $this->objectTypeLower . '_display', $urlArgs);
            }
    ...
    public function fetchInputData()
    {
        ...
        if (isset($this->form['additionalNotificationRemarks']) && $this->form['additionalNotificationRemarks']->getData() != '') {
            $this->requestStack->getCurrentRequest()->getSession()->set('MUBloggingModuleAdditionalNotificationRemarks', $this->form['additionalNotificationRemarks']->getData());
        }
